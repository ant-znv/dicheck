...
- `run.py` + `DI_Check.spec` + `build_exe.bat` — портативная сборка (PyInstaller onedir, windowed — консоли в exe нет): exe со встроенными фронтендом и промтом; `build_exe.bat` собирает `dist\DI_Check\` и `DI_Check_portable.zip`. В frozen-режиме ресурсы берутся из `sys._MEIPASS` (см. FRONTEND_DIST в main.py). `run.py` (с версии 0.2.0): GUI-режим по умолчанию — окно pywebview/WebView2, сервер FastAPI в фоновом потоке, закрытие окна останавливает приложение; `--console` — fallback (лог в консоль, браузер открывается сам; `--no-browser` действует только там). Если порт 8787 занят работающим DI_Check (`/api/settings` отвечает его JSON) — просто открывается окно к нему; чужой сервис на порту — сообщение об ошибке (в GUI — нативное окно, `_fatal`). При старте из `%TEMP%` удаляются установщики обновлений `di-check-setup-*.exe` старше 7 дней. GUI-зависимость: `pywebview>=5.0` (Windows).
- `backend/app/updater.py` — самообновление через GitHub Releases (check/install, внешние вызовы инъецируются; сетевые сбои install() → `UpdateError`; портативная сборка (нет `unins000.exe` рядом с exe) не обновляется автоматически); `DI_Check.iss` — установщик Inno Setup (per-user, чистка `_PYI_*`, taskkill без `/T`, `[InstallDelete]` для `{app}\_internal`, `[Run] skipifnotsilent`); `.github/workflows/build.yml` — CI: артефакты на push в main и PR в main, релиз на тегах `vX.Y.Z` (тег сверяется с `APP_VERSION`, зависимости — `requirements.lock.txt`). Релиз: поднять `APP_VERSION` — версия в `backend/app/version.py` (единственный источник) → тег `v*` → push.
