...
Заголовок `X-Fix-Results`: base64(utf-8 JSON `{"items": [{"filename": "<исходное имя>", "ok": true|false, "error": string|null, "method": "docx"|"text"|"llm"|null}]}`) — по одному элементу на каждый item в порядке запроса; `error` обрезается до 200 символов. Фронтенд использует его для сводки «Исправлено N из M» без распаковки архива; заголовок может отсутствовать у старых сборок.
Параметр `?save=true` — ответ вместо архива: 200 JSON `{"path": "<файл на диске>", "filename": "<имя>", "results": {"items": [та же сводка]}}`; архив сохраняется бэкендом в `saveDir` (см. `GET/PUT /api/settings`) с суффиксом-датой в имени. 500 — `save_failed: <причина>` (правки к этому моменту уже применены). Окно приложения использует save-режим по умолчанию; blob-ответ остаётся для браузерного режима и «скачать копию».
...
### Служебные: сохранённые артефакты

`GET /api/system/artifact?path=<файл>` — отдаёт ранее сохранённый файл на скачивание (Content-Disposition с его именем). `POST /api/system/reveal` `{ "path": "…" }` → `{"ok": true}` — открыть папку в Проводнике с выделенным файлом. Оба принимают **только пути внутри `saveDir`** — иначе 400 `invalid_path`; несуществующий файл — 404 `artifact_not_found`. Дефолт `saveDir`: `%USERPROFILE%\Downloads\DI_Check`, меняется через `PUT /api/settings` (`{"saveDir": "…"}`, пустая строка — вернуть дефолт).
